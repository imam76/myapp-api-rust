use sea_query::{Expr, Order, PostgresQueryBuilder, Query, SelectStatement, Iden};
use uuid::Uuid;
...
impl ContactQueryBuilder {
    ...
    fn build_select_query(workspace_id: Uuid, user_id: Uuid, filters: &ContactFilters) -> String {
        ...
        query
            ...
            .inner_join(
                Workspaces::Table,
                Expr::col((Contacts::Table, Contacts::WorkspaceId))
                    .equals((Workspaces::Table, Workspaces::Id))
            )
            .inner_join(
                WorkspaceUsers::Table,
                Expr::col((Workspaces::Table, Workspaces::Id))
                    .equals((WorkspaceUsers::Table, WorkspaceUsers::WorkspaceId))
            );
        ...
        let sort_order = if filters.sort_order == "ASC" {
            Order::Asc
        } else {
            Order::Desc
        };
